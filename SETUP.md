...
- **Workspace SSO for internal pages (two deployments)**: by default step 5 makes one deployment (*Execute as:* Me, *Who has access:* Anyone) that has to stay anonymous, since candidates and consulting-firm contacts — who submit through `Apply.html`/`Agency.html`/`SelfSchedule.html` — aren't `@healthy18.com` Google accounts and can't get past a domain-restricted login. To also require a real Google sign-in on the internal app, add a **second** deployment: **Deploy → New deployment → Web app**, *Execute as:* **User accessing the web app** (not "Me" — that setting is what actually lets the script see who's signed in), *Who has access:* **Anyone within Healthy18**. Share only this second URL with your team; keep sharing the original anonymous URL for candidate/agency links. Then add a Script Property `PUBLIC_APP_URL` set to the **original** (anonymous) deployment's URL — without it, self-serve scheduling links, consulting-firm portal links, and the public apply link would all get generated using whichever deployment happens to be serving the request, which breaks them the moment they're generated from the internal, domain-restricted deployment.
