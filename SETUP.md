...
- **Internal-only deployment (recommended for Healthy18)**: skip step 5's default entirely — this app never needs to face candidates or agencies directly. Candidates apply through a separate Google Form; consulting firms submit and track candidates through a shared Excel/Sheet, not through this app. Deploy with *Execute as:* **User accessing the web app** and *Who has access:* **Anyone within Healthy18** (domain-restricted — also set in `appsscript.json`'s `webapp` block: `"executeAs": "USER_ACCESSING", "access": "DOMAIN"`). Google's own domain-restricted sign-in wall means only an `@healthy18.com` account ever reaches `doGet()` at all, so there's no page-level access-control logic needed there and no `PUBLIC_APP_URL` property to keep in sync — one URL, one deployment, shared with your team only. `Apply.html`/`Agency.html`/`SelfSchedule.html` and their backend functions (sourcing-sheet sync, firm tokens, submission views) stay in the repo but are unreachable, in case candidate/agency self-serve through this app is revisited later. Because the deployment is domain-restricted, non-`@healthy18.com` Google accounts are refused by Google before any code runs, so there's nothing to verify empirically here. If a visitor still hits an "unverified app" consent warning, a Workspace admin can mark AgentATS as a trusted internal app under **Admin console → Security → API Controls → App access control** rather than going through Google's public OAuth verification process.
